...
- `models`: List models available from the current provider or a provider in `benchmark.yaml`.
- `results`: Generate markdown report from benchmark results.

```bash
uv run aoc-agent models
uv run aoc-agent models --provider copilot --config benchmark.yaml
uv run aoc-agent models --json
...
uv run aoc-agent results  # generates results/README.md
```
