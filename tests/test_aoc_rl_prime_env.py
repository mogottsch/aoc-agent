...
from aoc_rl.prime.env import AocPrimeToolEnv
...
def test_build_datasets_from_copied_cache_and_results(tmp_path: Path) -> None:
    ...
    train_ds, eval_ds = build_datasets(cache_dir=cache_dir, year=2022)
    ...
    assert row["day"] == 1
    assert row["answer"] == "3"
    assert row["prompt"][0]["content"] == build_agent_prompt(
        problem_html="<html>problem</html>",
        day=1,
        allow_sleep=False,
    )
...
def test_build_datasets_without_year_filter_includes_multiple_years(tmp_path: Path) -> None:
    ...
    for year, answer in ((2022, "3"), (2023, "7")):
        ...
        (year_dir / "day_1.part1_solved.html").write_text(
            ...
            f"<p>Your puzzle answer was <code>{answer}</code></p>"
        )

    train_ds, _ = build_datasets(cache_dir=cache_dir)
...
@pytest.mark.asyncio
async def test_prime_tool_env_runs_offline_with_wrapped_tools(
    ...
    def fake_description(_ctx: object) -> str:
        return "problem text"

    async def fake_execute(_ctx: object, code: str, **_: object) -> ExecuteResult:
        assert code == "print(3)"
    ...
    monkeypatch.setattr("aoc_rl.prime.env.get_aoc_data_service", lambda offline=True: FakeService())
    ...
    assert state["vf_reward"] == pytest.approx(1.0)
    assert state["vf_metrics"]["total_reward"] == pytest.approx(1.0)
