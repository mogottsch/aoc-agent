...
from aoc_agent.core.models import SolveStatus, SubmitLimitExceededError
...
def test_submit_same_incorrect_answer_three_times_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    submit_answer(ctx, 1, 123)
    with pytest.raises(SubmitLimitExceededError, match="same incorrect answer submitted 3 times"):
        submit_answer(ctx, 1, 123)
